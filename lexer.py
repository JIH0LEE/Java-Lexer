...
class Lexer():
    ...
    def make_token(self,input_str,line_num):
    ...
    def print_token(self,token_list,file):
    ...
    def check(self):
        ...
        try:
            readfile=open(self.file_name,'r')
        except:
        ...
        for data in lines:     
            for i in range(len(data)):
                token_str+=data[i]       
                temp_token=self.make_token(token_str,line_num)
                if temp_token!=None:                        
                    ...
                    if token_to_add ==None:
                        ...
                        if i== len(data)-1:         
                            self.token_lst.append(token_to_add)
            line_num+=1
        writefile=open(self.file_name+".out",'w')
        self.print_token(self.token_lst,writefile)
        writefile.close()   
